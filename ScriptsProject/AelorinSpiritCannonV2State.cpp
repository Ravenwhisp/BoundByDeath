...
AelorinSpiritCannonV2State::AelorinSpiritCannonV2State(GameObject* owner)
	: StateMachineScript(owner)
{
}

void AelorinSpiritCannonV2State::OnStateEnter()
{
	...
	if (m_activeAbility != AelorinAbility::SpiritCannonV2)
	{
...
void AelorinSpiritCannonV2State::OnStateUpdate()
{
	...
	if (m_shotActive)
	{
		...
		if (m_shotTimer < config->m_spiritCannonV2LockDuration)
		{
			return;
		}
	...
	if (m_waitingForNextShot)
	{
		...
		if (m_intervalTimer >= config->m_spiritCannonV2ShotInterval)
		{
			beginShot();
		}
	...
	if (m_recovering)
	{
		...
		if (m_recoveryTimer >= config->m_spiritCannonV2RecoveryDuration)
		{
			finishAbility();
		}
	}
...
void AelorinSpiritCannonV2State::OnStateExit()
{
	if (m_aelorinUI)
	{
...
void AelorinSpiritCannonV2State::selectTarget()
{
	if (!m_controller)
	{
		return;
...
bool AelorinSpiritCannonV2State::isValidTarget(Transform* targetTransform) const
{
	return m_attackExecutor && m_attackExecutor->isValidDamageTarget(targetTransform);
}

bool AelorinSpiritCannonV2State::lockCurrentTargetPosition()
{
	selectTarget();
...
void AelorinSpiritCannonV2State::beginShot()
{
	if (!m_controller)
	{
		return;
...
void AelorinSpiritCannonV2State::fireShot()
{
	...
	const Vector3 origin = TransformAPI::getGlobalPosition(m_aelorinTransform);
	m_controller->setSpiritCannonDebugLine(origin, m_lockedAimDirection, config->m_spiritCannonV2FireWidth);
	...
	m_attackExecutor->applyDamageInBeam(
		...
		m_lockedAimDirection,
		config->m_spiritCannonV2BeamLength,
		config->m_spiritCannonV2FireWidth,
		config->m_spiritCannonV2Damage,
		"Spirit Cannon"
	);
}

void AelorinSpiritCannonV2State::finishAbility()
{
	if (m_completed)
	{
		return;
...
IMPLEMENT_SCRIPT(AelorinSpiritCannonV2State)